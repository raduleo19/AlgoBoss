...
namespace WindowsFormsApplication4
{
    ...
    public partial class Grafuri : Form
    {
        ...
        private void GetQ()
        {
            JObject jResults = JObject.Parse(File.ReadAllText(@"Data/Questions.json"));
            JToken jarray = jResults["Q"];

            foreach (JObject it in jarray)
            {
                Intrebare newAns = new Intrebare();
                newAns.ID_Query = it["ID"].ToString();
                newAns.EN_Query = it["Q"].ToString();
                newAns.R1_Query = it["A1"].ToString();
                newAns.R2_Query = it["A2"].ToString();
                newAns.R3_Query = it["A3"].ToString();
                newAns.R4_Query = it["A4"].ToString();
                newAns.RS_Query = it["AC"].ToString();
        ...
        private void GetTheory()
        {
            ...
            for (int i = 1; i <= 18; i++)
            {
                String adress = @"Data/Theory" + "/" + i.ToString() + ".jpg";
                Theory[++MaxTProgress] = new Bitmap(adress);
            }
